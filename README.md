...
1. モデルを選んで読み込む（標準 0.5B / 日本語プラス 1.5B）  
...
2. 自動で次が行われます  
   - 依存関係のインストール  
   - **既定モデル**取得（0.5B・約 300 MB・初回のみネット必要）  
   - `dist/` への静的ビルド  
3. 終わると **`dist` フォルダ** がエクスプローラーで開きます

> この bat は「サイトを公開する人」専用です。訪問者は使いません。  
> **日本語プラス (1.5B)** も同梱したい場合は、bat の前か後に `npm run fetch-model:plus`（または `fetch-model:all`）を実行してから `npm run build` し直してください。両方で合計 ≈ **1.1 GB**。

#### 代わりにエージェント / 手動（上級者）

```bash
npm install
npm run fetch-model          # 既定 0.5B（必須・Pages 向け）
# npm run fetch-model:plus   # 任意: 日本語プラス 1.5B（≈830 MB）
# npm run fetch-model:all    # 両方（≈1.1 GB）
npm run build
```
...
|------|--------------|-----------|--------------|
| **既定のみ (0.5B)** | ≈ **300 MB** | ≈ 950 MB | **推奨**（1 GB ソフト枠内） |
| プラスのみ (1.5B) | ≈ **830 MB** | ≈ 1.6 GB | きつめ・非推奨 |
| **両方** | ≈ **1.1 GB** | 選んだ方 | **快適枠超過** → Netlify 等へ |
...
- **標準 (0.5B)** — 高速・既定  
- **日本語プラス (1.5B)** — より自然な日本語（未配置ならグレーアウト＋ `npm run fetch-model:plus` 案内）
...
2. プラスも載せるなら先に `npm run fetch-model:plus`（または `prepare-publish:all`）  
...
**既定モデル（約 300 MB）のみ** git に入れず、ワークフローがビルド時に `npm run fetch-model` で取得し、`dist/` ごと Pages に載せます。  
**プラス (1.5B) は CI では取得しません**（両方 ≈ 1.1 GB で Pages の快適枠を超えやすいため）。プラスが必要ならローカル／Netlify で `fetch-model:plus` してフルデプロイしてください。UI ではプラス未配置時にグレーアウトされます。
...
| 項目 | 目安 |
|------|------|
| 公開サイトサイズ（既定のみ） | 公式上限 **1 GB**。既定構成 ≈ **300 MB** |
| 両方デプロイ | ≈ **1.1 GB** → Pages ソフト枠超過。**Netlify 等を推奨** |
...
- プラス込みは Netlify / 自前ホスト向け（CI は既定のみ）
...
| 項目 | 内容 |
|------|------|
| 既定モデル | `Qwen2.5-0.5B-Instruct`（MLC q4f16_1）— 小型・高速 |
| プラスモデル | `Qwen2.5-1.5B-Instruct`（MLC q4f16_1）— より自然な日本語（任意） |
| ランタイム | `@mlc-ai/web-llm` **0.2.84** + **WebGPU**（Chrome / Edge 推奨） |
| 通信 | **プレイ中は外部ネットワーク不要**（同一オリジンの `models/` のみ） |
| 容量 | 既定 ≈ **300 MB** / プラス ≈ **830 MB** / 両方 ≈ **1.1 GB** |
| VRAM | 既定 ≈ **950 MB** / プラス ≈ **1.6 GB** |
...
│   ├── llm.js              # 同一オリジン WebLLM ローダ（標準/プラス切替）
...
| `Qwen2.5-0.5B-Instruct` 重み | **Apache-2.0** | [Qwen/Qwen2.5-0.5B-Instruct](https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct) |
...
| `Qwen2.5-1.5B-Instruct` 重み（プラス） | **Apache-2.0** | [Qwen/Qwen2.5-1.5B-Instruct](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct) |
...
3. サイト上かクレジット欄で、少なくとも次を示す:  
   - モデル: Qwen2.5-0.5B-Instruct（および使用する場合 1.5B）— © Alibaba Cloud — Apache-2.0  
   - ランタイム: WebLLM / MLC — Apache-2.0  
4. 「Qwen」「Alibaba Cloud」「MLC」等の**商標を、公式製品であるかのように勝手に使わない**  
5. モデルを「自分の独自学習成果」と偽らない／ライセンス表記を消さない  

> 注: Qwen2.5 の **3B / 72B** は別ライセンスです。本作品の **0.5B / 1.5B** は Apache-2.0 です。
...
```bash
npm install
npm run fetch-model        # 既定 0.5B → public/models（ネット一度きり）
npm run fetch-model:plus   # プラス 1.5B を追加
...
Windows でモデル取得だけ行う場合: `.\scripts\fetch-model.ps1`（引数 `plus` / `--all` 可）
