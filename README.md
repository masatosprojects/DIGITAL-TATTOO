# DIGITAL TATTOO — 伝言リレー

ブラウザ内で動く **バトン渡し（伝言ゲーム）** アート。  
小さな日本語対応 LLM が **推論のみ** で応答し、引き継ぎのたびに文脈が濁って「育ったように」見える。
...
- 「成長」「世代」は UI 上のメタファー
- 実際に増えているのは **蓄積ログ・割り込み燃料・プロンプトへの文脈汚染** と、ホップに応じたドリフト指示／温度
- エージェントが賢くなったように見えるのは、同じ小さなモデルが汚れた文脈を読み続けているから
...
1. **ユーザー割り込み** — 現バトン保持者が LLM で会話応答し、割り込みを混ぜた新バトンを生成  
2. **エージェント間バトン書き換え** — 引き継ぎ時に指令文を言い換え。ホップが増えるほどドリフト指示と温度が上がる  
