...
echo.
echo [2/4] モデルを取得しています（既定 0.5B・初回は約 300 MB）...
echo       ※ 実行中の作品は Hugging Face に接続しません。
echo       ※ ここで取得したファイルを dist/models に同梱します。
echo       ※ WebLLM 用に models/.../resolve/main/ 配置になります。
echo       ※ 日本語プラス (1.5B・約 830 MB) が必要なら別途:
echo         npm run fetch-model:plus   または  npm run fetch-model:all
echo       ※ 両方入れると合計 ≈ 1.1 GB（GitHub Pages の快適枠を超えがち）
...
echo      ・アップロードするのは dist の中身すべて（models 含む・既定 ≈ 300 MB）
echo      ・日本語プラスも入れる場合は先に npm run fetch-model:plus してから再ビルド（合計 ≈ 1.1 GB）
...
echo      ゲートで「標準 (0.5B)」か「日本語プラス (1.5B)」を選べます（プラス未配置ならグレーアウト）
echo.
